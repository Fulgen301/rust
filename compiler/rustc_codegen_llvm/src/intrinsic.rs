...
//
// This function is only generated once and is then cached.
fn get_rust_try_seh_fn<'a, 'll, 'tcx>(
    cx: &'a CodegenCx<'ll, 'tcx>,
    codegen: &mut dyn FnMut(Builder<'a, 'll, 'tcx>),
) -> (&'ll Type, &'ll Value) {
    if let Some(llfn) = cx.rust_try_seh_fn.get() {
        return llfn;
    }
    ...
    // `unsafe fn(*mut i8, i32, *mut i8) -> i32`
    let filter_fn_ty = Ty::new_fn_ptr(
        tcx,
        ty::Binder::dummy(tcx.mk_fn_sig(
            [i8p, tcx.types.i32, i8p],
            tcx.types.i32,
            false,
            hir::Safety::Unsafe,
            ExternAbi::Rust,
    ...
    let rust_try_seh = gen_fn(cx, "__rust_try_seh", rust_fn_sig, codegen);
    cx.rust_try_seh_fn.set(Some(rust_try_seh));
    rust_try_seh
}
...
fn declare_rust_try_seh_filter_fn<'a, 'll, 'tcx>(
    ...
    let fn_abi = cx.fn_abi_of_fn_ptr(rust_fn_sig, ty::List::empty());
    let llty = fn_abi.llvm_type(cx);
    let llfn = cx.declare_fn("?filt$0@0@__rust_try_seh@@", fn_abi, None);
    (llty, llfn)
}
...
fn get_rust_try_seh_filter_fn<'a, 'll, 'tcx>(
    ...
    if let Some(llfn) = cx.rust_try_seh_filter_fn.get() {
        return llfn;
    }
